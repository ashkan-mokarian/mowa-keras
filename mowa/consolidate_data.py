...
def main(dataset_dir, output_dir):
    ...
    # Randomize dataset train/val/test split based on 20/5/5
    if not use_fixed_split:
        wormnames = [os.path.basename(os.path.splitext(original_file)[0]) for
                    original_file in original_files]
        random.shuffle(wormnames)
        val_dataset_worm_names = wormnames[20:25]
        test_dataset_worm_names = wormnames[25:]
